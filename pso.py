...
class PSO():
  def __init__(self, X, func, model, lower_bound, upper_bound, device='cuda'):
    super().__init__()
    self.device = device  # Ensure that device is either 'cuda' or 'cpu'
    ...
    self.P = X.detach().to(self.device)  # Position of all particles for p_best
    self.P_best = torch.full((self.num_particle, 1), float('inf')).to(self.device)  # Best fitness of each particle
    self.G = None  # Position of a particle that has g_best
    self.global_best = torch.tensor(float('inf'), device=self.device)  # Best fitness value of all particles
  ...
  def initial_global_best(self):
      ...
      self.G = self.X.detach()[min_id].to(self.device)  # Ensure G is on the same device
      return self.G
